#!/bin/bash 
LOG=log/train_remap_global-`date +%Y-%m-%d_%H-%M-%S`-$MODEL.log 
...
$CAFFE train\
     --solver=solver_remap_global.prototxt \
     --weights=flow_iter_500000-remap-noscale.caffemodel,_iter_7500.caffemodel \
     --gpu=3 \
     2>&1 | tee $LOG
