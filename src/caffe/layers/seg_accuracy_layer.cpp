...
void SegAccuracyLayer<Dtype>::LayerSetUp(
  ...
  /// bear added.
  iter_ = 0;
  test_iter_ = seg_accuracy_param.test_iter();
...
void SegAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  */
  ...
  top[0]->mutable_cpu_data()[1] = (Dtype)confusion_matrix_.fg_accuracy(); 
  ...
  top[0]->mutable_cpu_data()[4] = (Dtype)confusion_matrix_.avgF1score();
  
  if (iter_ && iter_ == test_iter_ - 1)
    confusion_matrix_.printF1Score(NULL);
  
  iter_ = (iter_ + 1) % test_iter_;
}
